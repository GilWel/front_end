// Написать функцию, которая будет принимать 2 аргумента. Первый аргумент - массив, 
// где элементы это обьекты, описывающие неких людей. У каждого обьекта обязательно 
// есть свойство "name". Наших массив players как пример подходит. Второй аргумент - 
// это строка. Входящая строка должна состоять только из одной буквы.
// Функция должна вернуть массив из обьектов, отфильтрованных так, чтобы были только
//  обьекты где имя начинается на букву, которая переданна вторым аргументом.
...
let players = [
    ...
    const sortPlayers = (players, letter) =>
        
            
             players.filter(player => {

                return player.name.toLowerCase().startsWith(letter);

           
             });
  
                 console.log(sortPlayers(players,'o'));
